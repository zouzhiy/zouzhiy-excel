...
class DateNumberHandlerTest extends CellHandlerTest {

    private final DateNumberHandler cellHandler = new DateNumberHandler();

    @Override
    @Test
    void getJavaType() {
        Assertions.assertEquals(cellHandler.getJavaType(), Date.class);
    }
    ...
    void write() {
        LocalDateTime localDateTime = LocalDateTime.now();
        Date value = random.nextBoolean() ? null : this.convert(localDateTime).getTime();
