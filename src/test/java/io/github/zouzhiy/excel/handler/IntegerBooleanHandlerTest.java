...
class IntegerBooleanHandlerTest extends CellHandlerTest {

    private final IntegerBooleanHandler cellHandler = new IntegerBooleanHandler();


    @Override
    @Test
    void getJavaType() {
        Assertions.assertEquals(cellHandler.getJavaType(), Integer.class);
    }
    ...
    void readNone1() {
        ...
        Integer result = cellHandler.read(sheetContext, excelFieldConfig, cellResultSet);
    ...
    @RepeatedTest(10)
    void write() {
        Integer value = random.nextBoolean() ? null : random.nextInt();
        ...
        if (value == null) {
            ...
            Mockito.verify(cell).setCellValue(value.compareTo(1) == 0);
        }
